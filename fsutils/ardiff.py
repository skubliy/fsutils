...
def main_scenario(opts,args):
	...
	rroot, rdirs = opts.raw_root, opts.raw.split(',')
	...
	if dup_checker is not None:		
		...
		if fout_prefix or cout: 
			...
			with OutputSplitter([cout, prefix.get_full(fout_prefix, __SORTED_SUFFIX__)]) as outs:
				if outs.pipes:
					priners.print_sorted_by_catalogs_and_names("\n[SORTED DUPLICATES]\n",
										priners.group_by_folders(dup_checker.dup_files), 
										outs)
		...
		if fout_prefix or cout: 
			with OutputSplitter([cout, prefix.get_full(fout_prefix, __SORTED_SUFFIX__)]) as outs:
				if outs.pipes:
					...
					priners.print_sorted_by_catalogs_and_names("\n[UNIQS]\n",
															priners.group_by_folders(uniq_files), 
															outs)
